...
export default function App() {
  ...
  const topic = route.topic ? topicById(route.topic) : undefined;
  ...
  useEffect(() => { setNavOpen(false); document.title = topic ? `${topic.title} — The PM Playbook` : "The PM Playbook — product management, end to end"; }, [route.topic]);
      ...
      <div className="relative z-10 lg:grid lg:grid-cols-[280px_1fr]">
        ...
        <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[290px] flex-col border-r border-line-soft bg-ink-1/95 backdrop-blur-xl transition-transform duration-300 lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0 lg:bg-ink-1/60", navOpen ? "translate-x-0" : "-translate-x-full")} aria-label="Playbook navigation">
          <div className="flex items-center justify-between gap-3 border-b border-line-soft px-4 py-4">
            ...
            <button onClick={() => setNavOpen(false)} className="lg:hidden text-muted hover:text-text" aria-label="Close navigation">✕</button>
          </div>
          <div className="thin flex-1 overflow-y-auto px-3 py-3">
            <button onClick={() => nav(null)} className={cn("flex w-full items-center gap-3 rounded-lg border px-3 py-2 text-left transition-colors", !topic ? "border-line bg-ink-3 text-text" : "border-transparent text-dim hover:bg-ink-2 hover:text-text")}><span className="mono text-[.62rem] text-acc">00</span><span className="text-[.86rem] font-semibold">The loop<em className="block text-[.68rem] font-normal not-italic text-muted">home · all twelve</em></span></button>
          ...
          <div className="border-t border-line-soft p-3">
            <button onClick={toggle} className="flex w-full items-center justify-between rounded-lg border border-line-soft px-3 py-2 text-[.8rem] text-dim hover:text-text hover:border-line" aria-pressed={theme === "day"}><span>{theme === "night" ? "Night lab" : "Day desk"}</span><span className="text-acc">{theme === "night" ? "◐" : "◑"}</span></button>
          </div>
        </aside>
        ...
        <main id="content" className="min-w-0">
          <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-line-soft bg-ink/80 px-4 backdrop-blur-xl sm:px-6">
            <button onClick={() => setNavOpen(true)} className="lg:hidden flex flex-col gap-[5px] p-1" aria-label="Open navigation"><i className="block h-[2px] w-5 bg-text" /><i className="block h-[2px] w-5 bg-text" /><i className="block h-[2px] w-3 bg-text" /></button>
            <nav className="flex min-w-0 items-center gap-2 font-mono text-[.7rem] text-muted" aria-label="Breadcrumb">
              ...
              <span className="hidden sm:inline">{topic ? stageOf(topic.stage).label : "Home"}</span><span className="hidden sm:inline">/</span>
              <b className="truncate text-text">{topic ? `${topic.n} · ${topic.title}` : "The loop"}</b>
              {topic && activeSec && <span className="hidden md:inline truncate text-dim">/ {topic.sections.find((s) => s.id === activeSec)?.nav}</span>}
            </nav>
          ...
          <AnimatePresence mode="wait">
            {topic ? <TopicPage key={topic.id} t={topic} anchor={route.anchor} go={nav} /> : <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><Home go={nav} openSearch={() => setSearchOpen(true)} /></motion.div>}
          </AnimatePresence>
            ...
            <div className="mx-auto max-w-[1280px] px-5 py-14 sm:px-8">
              <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
                ...
                <div>
                  ...
                  <ul className="mt-3 space-y-1.5 text-[.86rem] list-none p-0 m-0">
                    <li><button onClick={() => nav(null)} className="text-dim hover:text-text">All topics</button></li>
                    {["ab-testing", "metrics", "fw-diagnosis", "fw-decide", "fw-deliver"].map((id) => { const t = topicById(id)!; return <li key={id}><button onClick={() => nav(id)} className="text-dim hover:text-text"><span className="mono text-[.66rem] text-acc mr-2">{t.n}</span>{t.title}</button></li>; })}
                  </ul>
                </div>
                <div>
                  <h4 className="font-mono text-[.6rem] uppercase tracking-[.2em] text-muted">Keyboard</h4>
                  <ul className="mt-3 space-y-2 text-[.82rem] text-dim list-none p-0 m-0">
              ...
              <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-line-soft pt-6 font-mono text-[.64rem] text-muted">
                <span>© 2026 · The PM Playbook</span><span>Version 4.0 · React build</span>
